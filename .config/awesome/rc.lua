...
-- new wallpaper
beautiful.wallpaper=("/home/jorge/")
...
timeLeft = wibox.widget.textbox()

function whereami()
	local fd=io.popen("whoami")
	local status = fd:read("*line")
	fd:close()
	fd=io.popen("hostname")
	local status2=fd:read("*all")
	fd:close()
	mytextbox:set_text(status .. "@" .. status2)
end
...
for s = 1, screen.count() do
    ...
    right_layout:add(mytextbox) -- TextBox
    -- right_layout:add(volume_widget) -- Volume shower
    right_layout:add(mytextclock)
    right_layout:add(timeLeft)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey            }, "F12", function() awful.util.spawn(lock_cmd) end),
